const INFO = {
	main: {
		title: "Portfolio by selinankuna",
		name: "Selina Nkuna",
		email: "selinankunas@gmail.com",
	},

	socials: {
		github: "https://github.com/",
		instagram: "https://instagram.com/",
		facebook: "https://facebook.com/",
	},

	homepage: {
		title: "Hi, my name is Selina Nkuna!",
		description:
			"I joined the Digital Academy 2years ago on the 1st of june 2021. I had basic IT skills from as college in sandton. When i joined the program here at the Digital Academy i learned more than just basic, i learned CSS,HTML,JS, BOOTSRAP and more.",
	},

	about: {
		title: "I’m Selina Nkuna. I live in Westonaria, where I design the future.",
		description:
			"I enjoy working and learning the basics. Thank you for the opportunity to work at Digital Academy. I have experience in working with junior web developers to create new projects, and I am excited to take on new challenges in my work.",
	},

	projects: [
		{
			title: "HTML",
			logo: "https://img.icons8.com/color/48/html-5--v1.png",
			description:"The HyperText Markup Language or HTML is the standard markup language for documents designed to be displayed in a web browser.",
		},

		{
			title: "CSS",
			logo: "https://img.icons8.com/color/48/css3.png",
			description:"hCascading Style Sheets is a style sheet language used for describing the presentation of a document written in a markup language such as HTML or XML.",
		},

		{
			title: "JavaScript",
			logo: "https://cdn.jsdelivr.net/npm/programming-languages-logos/src/javascript/javascript.png",
			description:"JavaScript, often abbreviated as JS, is a programming language that is one of the core technologies of the World Wide Web, alongside HTML and CSS.",
		},

		{
			title: "React.js",
			logo: "https://img.icons8.com/color/48/react-native.png",
			description:"React is a free and open-source front-end JavaScript library for building user interfaces based on components.",
		},

		{
			title: "Discord",
			logo: "https://img.icons8.com/color/48/discord--v2.png",
			description:"Discord is a VoIP and instant messaging social platform. Users have the ability to communicate with voice calls, video calls, text messaging,",
		},

		{
			title: "Bootstrap 5",
			logo: "https://img.icons8.com/color/48/bootstrap.png",
			description:"Bootstrap is a free and open-source CSS framework directed at responsive, mobile-first front-end web development.",
		},
	],
};

export default INFO;
